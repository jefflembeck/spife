...
module.exports = class Server {
  ...
  onrequest (req, res) {
    ...
    const sreq = new SpifeRequest(req, this, parsed)
    const session = hooks.startSession()
    session.request = sreq
    this.inneronrequest(sreq, res, session)
  }

  async inneronrequest (sreq, res, session) {
    var response
    try {
      response = handleResponse(this, sreq, await this.processRequestOnion(sreq))
    } catch (err) {
      response = handleLifecycleError(this, sreq, err)
    }
