...
const TEMPLATE_SYM = Symbol.for('spife-http-template')
const STATUS_SYM = Symbol.for('spife-http-status')
const HEADER_SYM = Symbol.for('spife-http-header')
const COOKIE_SYM = Symbol.for('spife-http-cookie')
...
class ImmediateStreamResponse {
  constructor (payload, status, headers) {
    this.payload = payload
    this[STATUS_SYM] = status
    this[HEADER_SYM] = headers
  }
...
function toStream (data) {
  ...
  if (isBuffer) {
    ...
    return new ImmediateStreamResponse(out, status || 200, headers)
  }

  return new ImmediateStreamResponse(
    JSON.stringify(out),
    status || 200,
    headers
  )
...
function empty (code) {
  return new ImmediateStreamResponse('', code || 204, {})
}
...
function setCookie (resp, name, value, options) {
  ...
  const cookie = (
    resp[COOKIE_SYM]
    ? resp[COOKIE_SYM]
    : (resp[COOKIE_SYM] = new CookieMap(resp))
  )
...
function _renderSetCookieHeader (cookieObject) {
  const resp = cookieObject.resp
  ...
  if (result.length === 0) {
    const headerObj = resp[HEADER_SYM] || {}
    delete headerObj['set-cookie']
  } else {
    header(resp, 'set-cookie', result)
  }
}
